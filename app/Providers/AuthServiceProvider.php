<?php

namespace App\Providers;

use App\Models\User;
use App\Support\Aitg\AitgMenuAccess;
use Illuminate\Foundation\Support\Providers\AuthServiceProvider as ServiceProvider;
use Illuminate\Support\Facades\Gate;

class AuthServiceProvider extends ServiceProvider
{
    /**
     * The model to policy mappings for the application.
     *
     * @var array<class-string, class-string>
     */
    protected $policies = [
        \App\Models\RedConocimiento::class => \App\Policies\RedConocimientoPolicy::class,
        \App\Models\Aprendiz::class => \App\Policies\AprendizPolicy::class,
        \App\Models\ProgramaFormacion::class => \App\Policies\ProgramaFormacionPolicy::class,
        \App\Models\FichaCaracterizacion::class => \App\Policies\FichaCaracterizacionPolicy::class,
        \App\Models\Persona::class => \App\Policies\PersonaPolicy::class,
        \App\Models\Aitg\Banco\DocumentoBanco::class => \App\Policies\Aitg\Banco\DocumentoBancoPolicy::class,
    ];

    /**
     * Register any authentication / authorization services.
     */
    public function boot(): void
    {
        Gate::define('ver-menu-aitg', function (?User $user) {
            return app(AitgMenuAccess::class)->shouldShowSidebarMenu($user);
        });

        Gate::define('ver-menu-aitg-banco', function (?User $user) {
            return $user?->can('VER BANCO INSTRUCTOR AITG')
                && app(AitgMenuAccess::class)->shouldShowSidebarMenu($user);
        });
    }
}

